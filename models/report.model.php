...
class ReportModel {
	...
	static public function mdlShowReportDataPariwisata ($tableDataPariwisata,$tableRefKodeData,$tahun, $bulan,$lokasi){
			// var_dump($lokasi);
		if($bulan != null){
			$stmt = Connection::connect()->prepare("SELECT id, a".".kode_data,keterangan ,kuantitas , status_persetujuan ,satuan 
					from $tableDataPariwisata a ,  $tableRefKodeData b 
					where tahun =$tahun and bulan =$bulan and a".".kode_data = b".".kode_data 
					and kode_lokasi =$lokasi and status_persetujuan= 1
					and a.kode_data LIKE 'B__'
					
					union

					SELECT id, a".".kode_data,keterangan ,kuantitas , status_persetujuan ,satuan 
					from $tableDataPariwisata a ,  $tableRefKodeData b 
					where tahun =$tahun and a".".kode_data = b".".kode_data 
					and kode_lokasi =$lokasi and status_persetujuan= 1
					and a.kode_data LIKE 'T__'
			...
			$stmt = Connection::connect()->prepare("SELECT keterangan , sum(kuantitas) kuantitas , status_persetujuan, satuan from $tableDataPariwisata a ,  $tableRefKodeData b where tahun =$tahun and a".".kode_data = b".".kode_data and status_persetujuan= 1 and kode_lokasi =$lokasi
							group by keterangan , status_persetujuan ,satuan" );
			// var_dump($stmt);die;
			$stmt -> execute();
	...
	static public function mdlShowReportDataPariwisataProvinsiExport2 ($tableDataPariwisata,$tableRefKodeData,$tahun,$bulan,$lokasi){
		if($bulan!= 13){
			if($lokasi == 99 ){
				$stmt = Connection::connect()->prepare("SELECT  keterangan ,SUM(kuantitas) kuantitas,satuan 
				from $tableDataPariwisata a ,  $tableRefKodeData b 
				where bulan =$bulan and a.kode_data = b.kode_data and status_persetujuan= 1 
				and tahun=$tahun GROUP by keterangan , satuan"  );
			} else {
				$stmt = Connection::connect()->prepare("SELECT id, a.kode_data,keterangan ,kuantitas , status_persetujuan ,satuan 
				from $tableDataPariwisata a ,  $tableRefKodeData b where bulan =$bulan 
				and tahun=$tahun and a.kode_data = b.kode_data 
				and status_persetujuan= 1 and kode_lokasi=$lokasi" );
			}
